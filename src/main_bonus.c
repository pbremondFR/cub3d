...
/*   Updated: 2024/12/18 17:09:48 by pbremond         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	_init_vars_and_hooks(t_game *g, t_cub *c)
{
	...
	c_load_font(g);
	mlx_hook(g->mw, E_KDWN, KeyPressMask, &c_keypress_handler, g);
	mlx_hook(g->mw, E_KUP, KeyReleaseMask, &c_keyrelease_handler, g);
	mlx_hook(g->mw, E_MPRESS, ButtonPressMask, &c_mousepress_handler, g);
	mlx_hook(g->mw, E_MREL, ButtonReleaseMask, &c_mouserelease_handler, g);
